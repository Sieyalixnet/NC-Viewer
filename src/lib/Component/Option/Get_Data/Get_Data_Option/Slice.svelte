<script>
    ...
    let confirm = async () => {
        slice=[]
        variable=""
        await tick()//should destroy all the children component and reload them, or it will throw an error because of {#each} in component
    ...
    let get_data = () => {
        let information_pass_geovector = {slice_helpers,dimensions_name:variable.dimensions_name,dimensions_len:variable.dimensions_len}
        dispatch("get_data", { slice: slice, name: variable.name,information_pass_geovector });
    };
</script>
