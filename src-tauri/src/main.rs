...
fn main() {
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![greet, readfile, get_history,get_attributes,get_variables,get_values,save_values,add_history,remove_file])
        //
        .setup(|app| {
...
#[tauri::command]
fn remove_file(index: usize, state: tauri::State<RustState>) {
    let mut state_guard = state.0.lock().unwrap();
    state_guard.remove_file(index);
}
#[tauri::command]
